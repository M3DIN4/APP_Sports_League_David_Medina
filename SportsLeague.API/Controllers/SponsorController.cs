...
public class SponsorController : ControllerBase
{
    ...
    [HttpGet]
    public async Task<ActionResult<IEnumerable<PatronResponseDto>>> GetAll()
    {
        var sponsors = await _sponsorService.GetAllAsync();
        return Ok(_mapper.Map<IEnumerable<PatronResponseDto>>(sponsors));
    }

    [HttpGet("{id}")]
    public async Task<ActionResult<PatronResponseDto>> GetById(int id)
    {
        var sponsor = await _sponsorService.GetByIdAsync(id);
        if (sponsor == null)
            return NotFound(new { message = $"Patrocinador con ID {id} no encontrado" });
        return Ok(_mapper.Map<PatronResponseDto>(sponsor));
    }

    [HttpPost]
    public async Task<ActionResult<PatronResponseDto>> Create(PatronRequestDto dto)
    {
        try
        {
            var sponsor = _mapper.Map<Sponsor>(dto);
            var created = await _sponsorService.CreateAsync(sponsor);
            var response = _mapper.Map<PatronResponseDto>(created);
    ...
    [HttpPut("{id}")]
    public async Task<ActionResult> Update(int id, PatronRequestDto dto)
    {
        try
        {
            var sponsor = _mapper.Map<Sponsor>(dto);
